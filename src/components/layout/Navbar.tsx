    ...
    <nav className="sticky top-0 z-50 bg-white border-b border-gray-100">
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-gray-100 bg-white px-4 py-4 space-y-3">
          <Link href="/explore" className="block text-sm text-gray-600" onClick={() => setMenuOpen(false)}>
            Explore
          </Link>
          {session ? (
            <>
              <Link href="/dashboard" className="block text-sm text-gray-600" onClick={() => setMenuOpen(false)}>
                Dashboard
              </Link>
              <Link href="/social" className="block text-sm text-gray-600" onClick={() => setMenuOpen(false)}>
                Social
              </Link>
              <button onClick={() => signOut({ callbackUrl: "/" })} className="block text-sm text-gray-600">
                Sign out
              </button>
            </>
          ) : (
